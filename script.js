...
function getWeatherData (clickedCity) {
    ...
    //$("#citySearch").val('')
    var dailyQueryURL = "https://api.openweathermap.org/data/2.5/forecast/daily?q=" + city + "&units=metric&cnt=6&appid=" + APIKey;
    $.ajax({
        url: dailyQueryURL,
        method: "GET",
...
function setCityValues(tempToday, city, humidityToday, windToday, icon) {
    ...
    $("#city").text(city + " " + date)
    $("#temp").text("Nhiệt độ: " + tempToday + " °C")
    $("#humidity").text("Độ ẩm: " + humidityToday + "%")
    $("#wind").text("Sức gió: " + windToday + " m/s")
...
function setForcastValues(icon, temp, humidity, day){
    ...
    $("#day"+day+"Temp").text("Nhiệt độ: " + temp + " °C")
    $("#day"+day+"Humidity").text("Độ ẩm: " + humidity + "%")
